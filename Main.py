...
def do_it(img, layers, num, inst, w, th, p, par, train, seed):
    ...
    pop.run(max([max(inp) for inp in input]) + 100, path='network_plots/',
            name=name,
            record_PG=True, save_post_model=False, PG_duration=50, PG_match_th=th, save_data=False, save_delays=False)
